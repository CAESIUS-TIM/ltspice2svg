...
def setPen(lineWidth=0.3, ISside=4, PPradius=2):
    ...
    if type(lineWidth) != float and type(lineWidth) != int:
        raise (
            TypeError(
                "Line width thickness must be a float or integer from 0 to 5 except 0. [Units: pt]"
            )
        )
    ...
    if type(ISside) != float and type(ISside) != int:
        raise (
            TypeError(
                "Intersection spot square side must be a float or integer from 1 to 20. [Units: pt]"
            )
        )
    ...
    if type(PPradius) != float and type(PPradius) != int:
        raise (
            TypeError(
                "Pin point radius must be a float or integer from 1 to 20. [Units: pt]"
            )
        )
...
def setFont(
    ...
    if type(pinFontSize) != int:
        raise (
            TypeError("Pin font size must be from 1 to 144 Integer only. [Units: pt]")
        )
    if pinFontSize < 1 or pinFontSize > 144:
        raise (
            ValueError("Pin font size must be from 1 to 144 Integer only. [Units: pt]")
        )
    if type(flagFontSize) != int:
        raise (
            TypeError("Flag font size must be from 1 to 144 Integer only. [Units: pt]")
        )
    if flagFontSize < 1 or flagFontSize > 144:
        raise (
            ValueError("Flag font size must be from 1 to 144 Integer only. [Units: pt]")
        )
...
def colour(
    isBW=False,
    symbColor="#000000",
    symbTextColor="#42f5aa",
    symbAttrColor="#f59e42",
    symbPinColor="#8742f5",
    symbFillColor="#fff9eb",
    flagColor="#77fc03",
    gndColor="#fc0352",
    comColor="#03bafc",
    wireColor="#524e63",
    ...
    if type(isBW) != bool:
        raise (
            ValueError(
                "isBW (is Black and White) parameter must be either True and False. If isBW is set to True then other parameters have no effect."
            )
        )
...
def _line(ix, iy, fx, fy, colour, dash="none"):
    ...
    dwg.add(
        dwg.line(
            ...
            fill="none",
            stroke_width=width,
            stroke_dasharray="{dashes}".format(dashes=dash),
        )
    )
...
def _arc(cx1, cy1, cx2, cy2, ix, iy, fx, fy, colour, dash="none"):
    ...
    dwg.add(
        dwg.path(
            d="M{},{} A{},{} {} {},{} {},{}".format(ix, iy, rx, ry, 0, l, 0, fx, fy),
            stroke=colour,
            fill="none",
            stroke_width=width,
            stroke_dasharray="{dashes}".format(dashes=dash),
...
def _squareSpot(x, y, colour):
    dwg.add(
        dwg.rect(
            (x - intersection_spot_side / 2, y - intersection_spot_side / 2),
            (intersection_spot_side, intersection_spot_side),
            stroke="none",
            fill=colour,
        )
    )
...
def _spot(x, y, colour):
    dwg.add(
        dwg.circle(
            (x, y), pin_spot_radius, stroke="none", fill=colour, stroke_width=width
        )
    )
...
def _text(x, y, string, size, weight, family, rAngle, colour):
    dwg.add(
        dwg.text(
            string,
            (x, y),
            font_size="{size}px".format(size=size),
            font_weight=weight,
            font_family=family,
            stroke="none",
            fill=colour,
            transform="rotate({rAngle} {xRotate} {yRotate})".format(
                rAngle=rAngle, xRotate=x, yRotate=y
            ),
        )
    )
...
):  # text width in pixel, size in px from 1 to 144, weight only normal or bold
...
class Coordinate:  # Coordinates are in px
    ...
    def rotate(self, theta):
        x1 = (self.x * math.cos(math.radians(theta))) - (
            self.y * math.sin(math.radians(theta))
        )
        y1 = (self.y * math.cos(math.radians(theta))) + (
            self.x * math.sin(math.radians(theta))
        )
...
def _leftBottomC(coordinate, textAlignment, textSize, tw):
    if textAlignment == "Left":
        ...
        leftBottom = Coordinate(
            coordinate.getX() - tw, coordinate.getY() + textSize * 0.5
        )
    elif textAlignment == "Center":
        # midMidC=Coordinate
        leftBottom = Coordinate(
            coordinate.getX() - tw * 0.5, coordinate.getY() + textSize * 0.5
        )
    elif textAlignment == "Top":
        # midTopC=Coordinate
        leftBottom = Coordinate(
            coordinate.getX() - tw * 0.5, coordinate.getY() + textSize
        )
        ...
        # topMidC=Coordinate
        leftBottom = Coordinate(
            coordinate.getX() + textSize * 0.5, coordinate.getY() + tw
        )
        ...
        # midMidC=Coordinate
        leftBottom = Coordinate(
            coordinate.getX() + textSize * 0.5, coordinate.getY() + tw * 0.5
        )
    elif textAlignment == "VTop":
        # leftMidC=Coordinate
        leftBottom = Coordinate(
            coordinate.getX() + textSize, coordinate.getY() + tw * 0.5
        )
...
def _DirFlag(Point, ContainerMax, ContainerMin, directionIndex, flagColour):
    ...
    if directionIndex == 1:
        ...
        PointM = Coordinate(
            ContainerMax.getX() + ContainerMin.getX() - Point.getX(), Point.getY()
        )
        _line(
            PointM.getX(),
            PointM.getY(),
        ...
        PointM = Coordinate(
            ContainerMin.getX() - ContainerMin.getX() + Point.getX(), Point.getY()
        )
        _line(
            PointM.getX(),
            PointM.getY(),
        ...
        PointM = Coordinate(
            Point.getX(), ContainerMax.getY() + ContainerMin.getY() - Point.getY()
        )
        _line(
            PointM.getX(),
            PointM.getY(),
        ...
        PointM = Coordinate(
            Point.getX(), ContainerMin.getY() - ContainerMax.getY() + Point.getY()
        )
        _line(
            PointM.getX(),
            PointM.getY(),
...
def _symb(symbC, symbTheta, isMirror, windows, windowValues, filepath):
    ...
    while index < len(lines):
        ...
        if z[0] in extract:
            if "LINE" == z[0]:
                startC = _transform(
                    Coordinate(float(z[2]), float(z[3])), isMirror, symbTheta, symbC
                )
                endC = _transform(
                    Coordinate(float(z[4]), float(z[5])), isMirror, symbTheta, symbC
                )
                try:
                    containerY = _container(
                        startC.getY(), endC.getY(), containerYmax, containerYmin
                    )
                ...
                try:
                    containerX = _container(
                        startC.getX(), endC.getX(), containerXmax, containerXmin
                    )
                ...
                try:
                    containerY = _container(
                        corner1C.getY(), corner2C.getY(), containerYmax, containerYmin
                    )
                    containerYmax = containerY[0]
                    containerYmin = containerY[1]
                except NameError:
                ...
                try:
                    containerX = _container(
                        corner1C.getX(), corner2C.getX(), containerXmax, containerXmin
                    )
                    containerXmax = containerX[0]
                    containerXmin = containerX[1]
                except NameError:
                ...
                boxCorner1C = _transform(
                    Coordinate(float(z[2]), float(z[3])), isMirror, symbTheta, symbC
                )
                boxCorner2C = _transform(
                    Coordinate(float(z[4]), float(z[5])), isMirror, symbTheta, symbC
                )
                startC = _transform(
                    Coordinate(float(z[6]), float(z[7])), isMirror, symbTheta, symbC
                )
                endC = _transform(
                    Coordinate(float(z[8]), float(z[9])), isMirror, symbTheta, symbC
                )
                try:
                    containerY = _container(
                        boxCorner1C.getY(),
                ...
                symbAttrC = _transform(
                    Coordinate(float(z[1]), float(z[2])), isMirror, symbTheta, symbC
                )
                ...
                _text(
                    ...
                    rAngle,
                    symbTextColor_,
                )  # some discripancy here textSize should be in px but given in pt units appears as px
            elif "WINDOW" == z[0]:
                windowIndex = list(windows.keys())
                if int(z[1]) not in windowIndex:
                    windows[int(z[1])] = Window(
                        Coordinate(float(z[2]), float(z[3])), z[4], int(z[5])
                    )
            elif "SYMATTR" == z[0]:
                try:
                    correspondingWindow = windows[AttributeIndices[z[1]]]
                    alignment = _rotateAlign(
                        correspondingWindow.getAlignment(), symbTheta
                    )
                    alignment = _mirrorAlign(alignment, isMirror)
                    coordinate = _transform(
                        correspondingWindow.getCoordinate(), isMirror, symbTheta, symbC
                    )
                    ...
                    if alignment[0] != "V":
                        bbMin = Coordinate(
                            leftBottom.getX(), leftBottom.getY() - textSize
                        )
                        bbMax = Coordinate(leftBottom.getX() + tw, leftBottom.getY())
                        rAngle = 0
                    else:
                        bbMin = Coordinate(
                            leftBottom.getX() - textSize, leftBottom.getY() - tw
                        )
                ...
                roundPinSpot = Coordinate(
                    round(pinSpot.getX(), 3), round(pinSpot.getY(), 3)
                )
                ...
                if pinAlignment != "NONE":
                    ...
                    if xNext[0] == "PINATTR" and xNext[1] == "PinName":
                        ...
                        pinLoc = _transform(
                            _pinOffsetC(pinLoc, pinOffset, pinAlignment),
                            isMirror,
                            symbTheta,
                            symbC,
                        )
                        ...
                        tw = _tw_in_px(
                            pinName, pin_font_family, pin_font_size_pt, pin_font_weight
                        )
                        leftBottom = _leftBottomC(
                            pinLoc, pinAlignment, pin_font_size, tw
                        )
                        if pinAlignment[0] != "V":
                            rAngle = 0
                        else:
        ...
        while True:
            ...
            if x != "":
                ...
                if z[0] == "WIRE":
                    ...
                    _line(
                        start.getX(), start.getY(), end.getX(), end.getY(), wireColor_
                    )
                    if len(pIntsecLocations) != 0:
                        for intersectionC in list(pIntsecLocations.keys()):
                    ...
                    tw = _tw_in_px(
                        textString, font_family, _close(textSize), font_weight
                    )
                    ...
                    nGnCflags[Coordinate(float(z[1]), float(z[2]))] = [
                        z[3],
                        "",
                    ]  # alignment]
                    ...
                    _busTap(
                        float(z[1]), float(z[2]), float(z[3]), float(z[4]), busTapColor_
                    )
            else:
                break
            lineNo += 1
